...
namespace Code{
    ...
    enum { CODE_ANSI_TRIGGER_1 = CODE_ESC, CODE_ANSI_TRIGGER_2 = CODE_LSQUAREBRACKET};
...
Color::Modifier Tile::tileColor (ull value){
    std::vector<Color::Modifier> colors{ red,yellow,magenta,blue,cyan,yellow,red,yellow,magenta,blue,green};
...
void Game::drawScoreBoard(std::ostream &out_stream)
{
    ...
    out_stream <<inner_border_padding << bold_on <<score_text_label << bold_off
               << std::string( inner_padding_length - std::string{score_text_label}.length()-
...
void Game::statistics(){
    ...
    endl();
    std::cout << "  largetst Tile:    " << bold_on <<largestTile <<bold_off;
    endl();
    std::cout << "  Number of moves:  " << bold_on << moveCount << bold_off;
    endl();
    std::cout << "  Time taken:       " << bold_on << secondsFormat(duration) << bold_off;
    endl();
}
...
void Game::setBoardSize()
{
    ...
    while(gameBoardPlaySize < MIN_GAME_BOARD_PLAY_SIZE ||
            ...
            std::cout <<bold_on 
                        << "   enter gameboard size (NOTE : Score and statistics will be "
                          " saved only for the 4×4 gameboard):  "
                          << bold_off;
